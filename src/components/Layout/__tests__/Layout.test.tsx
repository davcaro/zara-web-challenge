...
describe('Layout', () => {
  it('should render the marvel logo with a link to home', () => {
    const { getByLabelText, getAllByRole } = render(<Layout />);

    const logoLink = getAllByRole('link')[0];
    expect(logoLink).toHaveAttribute('href', '/');

    expect(getByLabelText('Marvel Logo')).toBeInTheDocument();
  });
  ...
  it('should render favorites count and a link to favorites page', () => {
    ...
    const { getByLabelText, getByText, getAllByRole } = render(<Layout />, {
      favoritesContext: mockFavoritesContextValue,
    });
